...
const withTimeout = <T,>(promise: Promise<T>, ms: number, code: LoginUiErrorCode) =>
  new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      const timeoutError = new Error(code) as Error & { code: LoginUiErrorCode };
...
const clearStaleAuthState = () => {
  try {
    ...
    console.log("[ClientLogin] cleared stale auth state", { count: keysToDelete.length });
  } catch (err) {
    console.error("[ClientLogin] failed to clear stale auth state", err);
  }
};
...
export default function ClientLogin() {
  ...
  const [errorCode, setErrorCode] = useState<LoginUiErrorCode | "">("");
  ...
  const reconcileSession = async () => {
    console.log("[ClientLogin] getSession start");
    const sessionRes = await withTimeout(supabase.auth.getSession(), SESSION_TIMEOUT_MS, "SESSION_TIMEOUT");
    console.log("[ClientLogin] getSession result", { hasSession: !!sessionRes.data.session });

    if (sessionRes.data.session) return sessionRes.data.session;

    console.log("[ClientLogin] refresh attempt");
    const refreshRes = await withTimeout(supabase.auth.refreshSession(), SESSION_TIMEOUT_MS, "SESSION_TIMEOUT");
    console.log("[ClientLogin] refresh result", { hasSession: !!refreshRes.data.session });

    if (refreshRes.data.session) return refreshRes.data.session;

    throw Object.assign(new Error("Session missing after token success"), { code: "SESSION_MISSING" as LoginUiErrorCode });
  };

  const normalizeError = (raw: unknown): { code: LoginUiErrorCode; message: string; retryable: boolean } => {
    const fallback = {
      code: "UNKNOWN_LOGIN_ERROR" as LoginUiErrorCode,
  ...
  const performLogin = async (allowAutoRetry = true) => {
    if (inFlightRef.current) {
      console.log("[ClientLogin] ignored duplicate submit");
      return;
    }
    ...
    try {
      console.log("[ClientLogin] login request start", { email });
      const { data, error: signInError } = await withTimeout(
        supabase.auth.signInWithPassword({ email, password }),
        LOGIN_TIMEOUT_MS,
        "LOGIN_TIMEOUT"
      );
      console.log("[ClientLogin] login request end", { hasError: !!signInError });

      if (signInError) throw signInError;

      console.log("[ClientLogin] token success", { userId: data.user?.id ?? null });
      await reconcileSession();

      if (!mountedRef.current) return;
      console.log("[ClientLogin] redirect event", { to: "/app/dashboard" });
      navigate("/app/dashboard", { replace: true });
      return;
    } catch (rawError) {
      const normalized = normalizeError(rawError);
      console.error("[ClientLogin] login flow error", {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        ...
        if (data.session) {
          console.log("[ClientLogin] existing session found, redirecting");
          navigate("/app/dashboard", { replace: true });
        }
      } catch (err) {
        console.warn("[ClientLogin] initial session check failed", err);
      }
    };
    ...
    <div style={{ minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ width: 400, padding: 40 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 11, color: T3, letterSpacing: 1.4, textTransform: "uppercase" }}>Client Login</div>
        </div>
        <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} style={IS} required disabled={loading} />
        ...
        <div style={{ textAlign: "center", marginTop: 16, display: "flex", flexDirection: "column", gap: 8 }}>
          <Link to="/app/forgot-password" style={{ color: T2, fontSize: 12, textDecoration: "none" }}>Forgot password?</Link>
