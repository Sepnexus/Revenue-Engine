-- ============================================================================
-- ghl_sync_finalize v9: keep legacy m1/m2/m3 fields in sync with channels[].
--
-- Bug found: the dashboard's Summary tab YTD/All-Time aggregation reads the
-- legacy m1ClosedRevenue / m2ClosedRevenue / m3ClosedRevenue fields directly
-- (not via getChannels()). My earlier finalize wrote channels[] but left the
-- legacy fields stale → headline numbers off by $286,101 for iBuyKC YTD.
--
-- Fix: after building channels[], also write the equivalent positional
-- m1/m2/m3 fields (alphabetical by channel name, just like the dashboard's
-- own syncChannelsToLegacy() helper). For orgs with >3 channels, only the
-- first 3 get into the legacy fields — same limitation the dashboard has.
-- ============================================================================
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      (   c.tags @> to_jsonb(v_tag_net)::jsonb
       OR c.tags @> to_jsonb(v_tag_offer)::jsonb
       OR c.tags @> to_jsonb(v_tag_contract)::jsonb
       OR c.tags @> to_jsonb(v_tag_closed)::jsonb) AS reached_net,
      ...
      (   c.tags @> to_jsonb(v_tag_contract)::jsonb
       OR c.tags @> to_jsonb(v_tag_closed)::jsonb) AS reached_contract,
  ...
  -- ─── Per-month upsert with merge-by-name + spend preservation + legacy sync ──
  FOR v_rec IN
    SELECT period_start,
           ...
           jsonb_agg(channel_name ORDER BY channel_name) AS ghl_channel_names,
  ...
  LOOP
    ...
    -- Build the legacy m1/m2/m3 patch (positional, alphabetical by name).
    -- The dashboard's Summary YTD uses these fields directly, so keeping
    -- them in sync prevents the headline-vs-channels discrepancy.
    WITH merged AS (
      SELECT row_number() OVER () AS pos, ch
      FROM jsonb_array_elements(v_merged_ghl_channels) ch
    )
    ...
    VALUES (p_org_id, v_rec.period_start,
            jsonb_build_object('channels', v_kept_channels || v_merged_ghl_channels) || v_legacy_patch,
            auth.uid())
