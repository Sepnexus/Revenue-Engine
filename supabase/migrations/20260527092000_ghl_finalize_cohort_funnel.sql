-- ============================================================================
-- ghl_sync_finalize v6: cohort-based funnel.
--
-- The earlier version bucketed each stage by the month of contact.updated_at,
-- which mixed cohorts and produced weird ratios like 540% "lead → net".
--
-- New model: for each month, the cohort = contacts created in that month
-- matching the channel's source rules. Each funnel column then counts how
-- many of THAT cohort currently have the corresponding funnel tag.
--
-- Result: newLeads ≥ netLeads ≥ offers ≥ contracts ≥ closedDeals (assuming
-- tags accumulate as a contact moves through the funnel — the usual CRM
-- workflow).
--
-- closedRevenue = sum(monetary_value) of WON opps for cohort contacts.
-- Revenue is attributed to the LEAD's creation month, not the close month.
-- That's the canonical "channel performance" view — answers "of leads
-- generated in May, how much revenue have they produced so far?"
-- ============================================================================
...
BEGIN
  ...
  -- ─── (contact → channel) lookup from source rules ──────────────────────
  ...
  -- ─── Cohort funnel: for each (month, channel), one row with all stages ──
  ...
  -- One pass, one row per (channel, month). Every funnel column counts
  -- members of the same cohort (contacts created in that month, matched to
  -- the channel) that carry the corresponding tag.
  ...
  SELECT
    ...
    cc.channel_id,
    cc.channel_name,
    count(*) FILTER (WHERE c.tags @> to_jsonb(v_tag_new)::jsonb)      AS new_leads,
    count(*) FILTER (WHERE c.tags @> to_jsonb(v_tag_net)::jsonb)      AS net_leads,
    count(*) FILTER (WHERE c.tags @> to_jsonb(v_tag_offer)::jsonb)    AS offers,
    count(*) FILTER (WHERE c.tags @> to_jsonb(v_tag_contract)::jsonb) AS contracts,
    count(*) FILTER (WHERE c.tags @> to_jsonb(v_tag_closed)::jsonb)   AS closed_deals
  FROM public.ghl_contacts c
  ...
  -- closedRevenue: sum of WON opps for the cohort's contacts.
  -- Attributed to the LEAD's creation month (not the opp's close month).
  WITH cohort_won AS (
    SELECT
      date_trunc('month', c.ghl_created_at)::date AS period_start,
      cc.channel_id,
